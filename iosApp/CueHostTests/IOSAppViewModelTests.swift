...
#if canImport(Cue)
@testable import Cue
#elseif canImport(CueHost)
@testable import CueHost
#endif
...
final class IOSAppViewModelTests: XCTestCase {
    ...
    func testStartDemoReflectsCurrentNotificationPermissionState() async {
        let service = FakeIOSAppService(
            startSession: AuthorizationSessionState(
                userCode: "ABCD-1234",
                verificationURL: "https://real-debrid.com/device",
        ...
        let permissionProvider = MutableNotificationPermissionStateProvider(state: .granted)
        ...
        await waitUntil {
            await MainActor.run { viewModel.isDemoMode }
        }

        await MainActor.run {
            XCTAssertEqual(.granted, viewModel.notificationPermissionState)
            XCTAssertFalse(viewModel.scheduledReminders.isEmpty)
        }
    }

    func testRefreshNotificationPermissionUpdatesAfterSettingsChange() async {
        let service = FakeIOSAppService(
            startSession: AuthorizationSessionState(
                userCode: "ABCD-1234",
                verificationURL: "https://real-debrid.com/device",
        ...
        let permissionProvider = MutableNotificationPermissionStateProvider(state: .denied)
        ...
        await waitUntil {
            await MainActor.run { viewModel.notificationPermissionState == .denied }
        }

        await permissionProvider.setState(.granted)
        await MainActor.run {
            viewModel.refreshNotificationPermission()
        }
        await waitUntil {
            await MainActor.run { viewModel.notificationPermissionState == .granted }
        }
...
private actor MutableNotificationPermissionStateProvider: NotificationPermissionStateProviding {
    private var state: NotificationPermissionState

    init(state: NotificationPermissionState) {
        self.state = state
    }
    ...
    func setState(_ newState: NotificationPermissionState) {
        state = newState
    }
}
